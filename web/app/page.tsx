...
export default function Home() {
  ...
  return <main dir="ltr"><header><div className="brand"><InstagramMark /><span>ιɴѕтαɢrαм αι coɴтrol ceɴтer ѕнιwα</span></div><span className="pill">Creator dashboard</span></header><div className="layout"><aside>
    ...
    <form className="card" onSubmit={submit}><textarea value={input} onChange={(event) => setInput(event.target.value)} placeholder={placeholders[view]} /><div className="actions"><button className="primary" disabled={busy || !input.trim()}>{busy ? 'Processing...' : view === 'chat' ? 'Send message' : view === 'commands' ? 'Run command' : view === 'content' ? 'Create content' : view === 'image' ? 'Generate image' : 'Search public results'}</button><button type="button" onClick={() => { setInput(''); setResult(null); setError(''); if (view === 'chat') setMessages([]); }}>Clear</button></div>{error && <div className="error">{error}</div>}{Boolean(result && typeof result === 'object' && result !== null && 'data_url' in result) ? <img className="generated" src={String((result as { data_url: unknown }).data_url)} alt="Generated result" /> : null}{result ? <pre>{JSON.stringify(result, null, 2)}</pre> : null}</form>
